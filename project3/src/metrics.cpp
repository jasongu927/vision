...
void findComponents(cv::Mat &src, cv::Mat &dest, bool visualize){
	cv::Mat tmp;
	int numLabels =	cv::connectedComponents(src, tmp, 4);
	std::cout << "input size is " << src.size() << " with type " << src.type() << std::endl;
	std::cout << "output size is " << dest.size() << " with type " << dest.type() << std::endl;
